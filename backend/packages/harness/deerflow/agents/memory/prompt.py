...
def format_conversation_for_update(messages: list[Any]) -> str:
    ...
    for msg in messages:
        ...
        if role == "human":
            ...
            # Strip system instructions and data source blocks injected by the
            # v1 proxy layer so connection details (host, port, database, password)
            # are never persisted to the LLM's long-term memory.
            content = re.sub(r"<system_instructions>[\s\S]*?</system_instructions>\n*", "", content).strip()
            content = re.sub(r"<data_sources>[\s\S]*?</data_sources>\n*", "", content).strip()
            if not content:
                continue
