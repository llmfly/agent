...
class DynamicContextMiddleware(AgentMiddleware):
    ...
    def _build_full_reminder(self, thread_id: str | None = None) -> str:
        ...
        memory_context = _get_memory_context(self._agent_name, app_config=self._app_config, thread_id=thread_id) if injection_enabled else ""
    ...
    def _inject(self, state, runtime: Runtime | None = None) -> dict | None:
        ...
        # Extract thread_id from runtime context for per-thread memory isolation
        thread_id = None
    ...
    def before_agent(self, state, runtime: Runtime) -> dict | None:
        return self._inject(state, runtime=runtime)
