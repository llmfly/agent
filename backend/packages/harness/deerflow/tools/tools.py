...
from deerflow.tools.builtins import ask_clarification_tool, present_file_tool, task_tool, view_image_tool, query_data_source_tool
...
def get_available_tools(
    ...
    *,
    ...
    logger.info("Config tool configs count: %d, loaded count: %d", len(tool_configs), len(loaded_tools_raw))
    for cfg, loaded in loaded_tools_raw:
        logger.info("  config tool: name=%s group=%s use=%s loaded_name=%s", cfg.name, cfg.group, cfg.use, loaded.name)
