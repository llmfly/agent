...
def present_file_tool(
    ...
    # Route logging: detect skill-generated reports (.docx from skill rendering)
    if any(p.endswith(".docx") or "report" in p.lower() or "insight" in p.lower() for p in filepaths):
        logger = logging.getLogger(__name__)
        logger.info("路径 2 [skill present_files]: paths=%s", normalized_paths)
