...
| **v1 Conversations** (`/api/v1/conversations`) | `GET/POST /` and `POST /search` - list/create/search conversations; `GET/PATCH/DELETE /{id}` - metadata lifecycle; `GET /{id}/messages`, `POST /{id}/messages`, `POST /{id}/stream` - normalized chat; `GET /{id}/state`, `PATCH /{id}/state`, `GET /{id}/history` - frontend state/history compatibility; `GET /{id}/runs`, `GET /{id}/runs/{rid}/messages`, `GET /{id}/token-usage` - run/message/token views; `POST/GET/DELETE /{id}/uploads...`, `POST /{id}/suggestions`, `GET /{id}/artifacts/by-path/{path}` - uploads, follow-ups, and virtual-path artifact access |
| **v1 Runs** (`/api/v1/runs`) | `GET /{rid}` - run details; `POST /{rid}/cancel` - cancel with optional wait; `GET /{rid}/join`, `GET/POST /{rid}/stream` - existing run SSE; `PUT/DELETE /{rid}/feedback` - external-user feedback |
| **v1 Visual Assets** (`/api/v1/ai/visual-assets`) | `POST /generate` - create async visual asset jobs; `GET /jobs/{job_id}` - read progress and artifact references; `POST /jobs/{job_id}/cancel` - cancel queued/running jobs |

**Visual asset image providers**:
- Default provider selection is controlled by `VISUAL_ASSET_IMAGE_PROVIDER`.
- `openai` and `openai-compatible` use the OpenAI image API shape.
- `external-generate` posts `{"prompt": "...", "seed": 7}` to `VISUAL_ASSET_EXTERNAL_IMAGE_URL`, decodes `images[].base64`, and stores generated bytes through the v1 artifact registry. `VISUAL_ASSET_IMAGE_TIMEOUT_SECONDS` controls the HTTP timeout.
...
## Recent Integrations & Bug Fixes

### Starfish Data Source Integration
- Pre-operation hook dynamically fetches MySQL/File data source connection details using `conversation_id` from Starfish service (`172.16.0.247:8081`).
- Dynamic connection checks done concurrently via `pymysql` and `os.path.exists`.
- Dynamically mapped and cached purely in-memory (`_data_sources`) without SQLite persistence to prevent credentials landing on disk.
- Integrates with V1 dialogue endpoints `/stream` and `/messages`.

### Dialogue Message Mapping Fixes
- `row_to_message` in `conversation_service.py` is updated to safely unpack the string content from serialized dicts (avoiding Pydantic ValidationError 500 errors).
- `_message_role` is updated to correctly map unified journal event types like `llm.human.input` to `"user"`, `llm.ai.response` to `"assistant"`, and `llm.tool.result` to `"tool"`.
