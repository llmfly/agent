...
class _HangingDisconnectRequest:
...
class TestWaitForRunCompletion:
    ...
    def test_stream_iterator_does_not_stall_when_disconnect_probe_hangs(self) -> None:
        """A stuck disconnect probe must not block heartbeat or END delivery."""
        from app.gateway.services import iter_run_stream_entries

        async def run() -> None:
            mgr = RunManager()
            bridge = MemoryStreamBridge()
            record = await _create_running_record(mgr, on_disconnect=DisconnectMode.cancel)
            request = _HangingDisconnectRequest()

            stream = iter_run_stream_entries(
                bridge,
                record,
                request,
                mgr,
