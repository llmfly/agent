...
_DISCONNECT_POLL_TIMEOUT_SECONDS = 0.1
...
async def _request_is_disconnected(request: Request) -> bool:
    """Poll request disconnect status without letting the stream loop stall."""
    try:
        return await asyncio.wait_for(request.is_disconnected(), timeout=_DISCONNECT_POLL_TIMEOUT_SECONDS)
    except TimeoutError:
        return False
...
def build_run_config(
    ...
    *,
    ...
    identically.
    """
    config: dict[str, Any] = {"recursion_limit": 1000}
    if request_config:
...
async def iter_run_stream_entries(
    bridge: StreamBridge,
    record: RunRecord,
    request: Request,
    run_mgr: RunManager,
    *,
    ...
    """Yield raw stream bridge entries while enforcing run lifecycle semantics.

    This is the common stream-consumption path for ``/api/threads`` and v1
    conversation streams.  Keeping END handling and disconnect cancellation in
    one place prevents wrapper APIs from leaving runs stuck in ``running``.
    ...
    completed = False
    try:
        async for entry in bridge.subscribe(record.run_id, last_event_id=last_event_id, heartbeat_interval=heartbeat_interval):
...
async def sse_consumer(
    ...
    async for entry in iter_run_stream_entries(bridge, record, request, run_mgr):
