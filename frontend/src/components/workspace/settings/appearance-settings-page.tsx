...
export function AppearanceSettingsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection
        title={t.settings.appearance.tokenTitle}
        description={t.settings.appearance.tokenDescription}
      >
        <TokenUsageSetting />
      </SettingsSection>
    </div>
...
const tokenViewOptions: { value: TokenUsageViewPreset; label: string }[] = [
  { value: "off", label: "关闭" },
  { value: "summary", label: "摘要" },
  { value: "per_turn", label: "每轮" },
  { value: "debug", label: "调试" },
];

function TokenUsageSetting() {
  const { t, locale } = useI18n();
  const [localSettings, setLocalSettings] = useLocalSettings();
  const currentView = getTokenUsageViewPreset(localSettings.tokenUsage) ?? "off";

  const labels: Record<TokenUsageViewPreset, string> = {
    off: locale === "zh-CN" ? "关闭" : "Off",
    summary: locale === "zh-CN" ? "摘要" : "Summary",
    per_turn: locale === "zh-CN" ? "每轮" : "Per Turn",
    debug: locale === "zh-CN" ? "调试" : "Debug",
  };
  ...
  return (
    <Select
      value={currentView}
      onValueChange={(value) => {
        setLocalSettings("tokenUsage", tokenUsagePreferencesFromPreset(value as TokenUsageViewPreset));
      }}
    >
      <SelectTrigger className="w-[220px]">
        <SelectValue />
