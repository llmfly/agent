...
#### Visual Asset Image Provider

The v1 visual asset API can use an external image generation service instead of the default OpenAI-compatible image endpoint. Configure the gateway environment like this:
...
The external service is called with JSON `{"prompt": "...", "seed": 7}`. DeerFlow decodes `images[].base64` from the response, stores the bytes in its artifact registry, and returns the normal `/api/v1/artifacts/{artifact_id}/preview` and `/download` URLs to clients.
