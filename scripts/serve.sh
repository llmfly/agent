...
echo ""
echo "  Services:"
echo "    Gateway     → localhost:8005  (REST API + agent runtime)"
...
# 1. Gateway API
run_service "Gateway" \
    "cd backend && PYTHONPATH=. uv run uvicorn app.gateway.app:app --host 0.0.0.0 --port 8005 $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1" \
    8005 30
...
echo "           /api/*              →  Gateway REST API (8005)"
echo ""
