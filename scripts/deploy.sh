...
TIKTOKEN_BUNDLE="$REPO_ROOT/scripts/deploy_resources/tiktoken/cl100k_base.tiktoken"
TIKTOKEN_HOST_CACHE_DIR="$DEER_FLOW_HOME/tiktoken-cache"
mkdir -p "$TIKTOKEN_HOST_CACHE_DIR"
if [ ! -s "$TIKTOKEN_HOST_CACHE_DIR/$TIKTOKEN_CACHE_KEY" ]; then
    if [ -s "$TIKTOKEN_BUNDLE" ]; then
        cp "$TIKTOKEN_BUNDLE" "$TIKTOKEN_HOST_CACHE_DIR/$TIKTOKEN_CACHE_KEY"
        echo -e "${GREEN}Seeded tiktoken cl100k_base cache${NC}"
    else
        echo -e "${YELLOW}tiktoken bundle not found; gateway may try to download cl100k_base at runtime.${NC}"
    fi
fi
